...
The authoritative derivation details for fixtures 39-42 are also recorded in `fixtures/v0.3/README.md`. No fixture is generated by the validator.
...
The expected new-side inventory is 42 `input.json` files, `42 * 13 = 546` golden files, and one README. The first 38 fixture directories correspond to the v0.2.1 set; fixtures 39-41 add the v0.3 minor-line inputs and 39 goldens, while fixture 42 adds the v0.3.1 patch input and 13 goldens. Git rename detection may pair identical metric-definition files across fixture numbers, so reconciliation uses the destination inventory plus the semantic ledger rather than rename similarity alone.
