...
Purpose is per-event and comes from the SDK's own purpose mapping (`attribution`, `analytics` are consent-required by contract guidance; `fraud_prevention` and `revenue_measurement` are not). The mapping must be a table in the SDK, not scattered `if`s, and the withdrawal test asserts the exact set that survives.
...
| `ReferrerDetails` reference | `https://developer.android.com/reference/com/android/installreferrer/api/ReferrerDetails` | Lists only `getInstallReferrer()`, `getReferrerClickTimestampSeconds()`, `getInstallBeginTimestampSeconds()`, `getGooglePlayInstantParam()`. **Getters for the two server timestamps and `install_version` are not shown on this page — unconfirmed at the Java API level.** The AIDL page above confirms the underlying data. Resolved by compiling (A-09b). |
...
| AppLovin MAX ILRD (Unity) | `https://support.applovin.com/en/max/unity/overview/advanced-settings` | Per-format `MaxSdkCallbacks.{Interstitial,Rewarded,Banner,MRec}.OnAdRevenuePaidEvent` with `(string adUnitId, MaxSdkBase.AdInfo adInfo)`. `AdInfo` fields `Revenue`, `RevenuePrecision`, `NetworkName`, `AdUnitIdentifier`, `Placement`, `NetworkPlacement`. |
...
1. Java getter names on `ReferrerDetails` for the two server timestamps and `install_version`. Resolved by compiling (A-09b).
