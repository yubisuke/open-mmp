...
| `sdk-ios` | IPA secret extraction, queue/credential leakage, restored identifiers, pre-consent Apple reads, callback lifetime misuse, privacy-manifest drift, and undeclared device API access | One excluded/protected storage subtree; WAL queue and process-death tests; `secure_delete`; HMAC shared vectors; collection-default and withdrawal gates; deletion-first reset; no second AdServices read; C ABI allocation table; built-object Required Reason/forbidden-symbol audit; tracking-disabled privacy manifest; dependency-empty runtime SBOM | **M4 does not prevent an attacker who possesses the IPA from enrolling installations or fabricating events.** App Attest, real backup/transfer behavior, App Store privacy review, live MAX, and Unity export remain M5/operator evidence. |

## Residual risk and release gates

M1-M4 now have local network services, a tenant database, authenticated admin path, envelope-encrypted protected-object store, Android/iOS SDKs, Apple receivers, and synthetic runtime security gates. They still cannot prove production TLS termination, external secret-manager operation, real provider/device delivery, capacity, availability, backup recovery, live fraud controls, App Store review, or incident response. Those remain operator and M5 gates.
