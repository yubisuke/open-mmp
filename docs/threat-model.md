...
M1-M6 now have local network services, a tenant database, authenticated and role-scoped admin paths, envelope-encrypted protected-object storage, Android/iOS SDKs, Apple receivers, deterministic fraud controls, synthetic platform-integrity adapters, restore/privacy/load gates, operational metrics, and release inventories. They still cannot prove production TLS termination, external secret-manager operation, real provider/device delivery, representative capacity, availability, backup operations, live integrity accuracy, App Store review, or incident response. Those remain operator gates.
