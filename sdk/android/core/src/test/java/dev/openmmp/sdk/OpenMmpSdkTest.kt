...
class OpenMmpSdkTest {
  private lateinit var context: Context
  private val created = mutableListOf<OpenMmpSdk>()

  @Before fun setUp() {
    context = ApplicationProvider.getApplicationContext()
    context.getSharedPreferences(OpenMmpStorage.PREFERENCES, Context.MODE_PRIVATE).edit().clear().commit()
    context.filesDir.resolve(OpenMmpStorage.SUBTREE).deleteRecursively()
  }
  ...
  @Test fun `withdrawal atomically purges consent-required purposes and keeps the control event`() {
    ...
    val fraudDatabase = OpenMmpQueueDatabase.open(context)
    try {
  ...
  ): OpenMmpSdk = OpenMmpSdk.create(
    context,
    OpenMmpConfiguration("http://127.0.0.1", "sdk-key:synthetic", "synthetic-secret"),
    transport,
    playReader,
    metaReader,
    {},
  ...
  private class RecordingTransport(private val deliverySucceeds: Boolean) : OpenMmpTransport {
