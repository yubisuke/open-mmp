import Foundation
import XCTest
@testable import OpenMmpCore
#if os(macOS)
import Darwin
#endif

final class OpenMmpCoreTests: XCTestCase {
  ...
  func testM4A21QueueSurvivesReopenWithoutDuplicates() throws {
    ...
    let reopened = try OpenMmpStorage(root: root)
  ...
  func testM4A22InfoPlistDefaultDisabledWinsBeforeInitialize() throws {
    ...
    let info: [String: Any] = [
      "CFBundleIdentifier": "dev.openmmp.synthetic-default",
      "CFBundleName": "OpenMmpSyntheticDefault",
      "CFBundlePackageType": "BNDL",
      "OpenMmpCollectionEnabledDefault": false,
    ]
    let data = try PropertyListSerialization.data(fromPropertyList: info, format: .xml, options: 0)
    try data.write(to: contents.appendingPathComponent("Info.plist"))
    let bundle = try XCTUnwrap(Bundle(url: bundleRoot))
    XCTAssertFalse(OpenMmpSDK.collectionDefault(bundle: bundle, fallback: true))
  }
  ...
  private func configuration(defaultEnabled: Bool) -> OpenMmpConfiguration {
    OpenMmpConfiguration(
  ...
  private func temporaryDirectory(_ name: String) -> URL {
    let url = FileManager.default.temporaryDirectory
      .appendingPathComponent("openmmp-\(name)-\(UUID().uuidString)", isDirectory: true)
...
private actor RecordingTransport: OpenMmpTransport {
  private var calls = 0
  ...
  func deliver(credential: InstallationCredential, events: [QueuedEvent]) async throws {
    ...
    if failDelivery { throw OpenMmpError.transport(503) }
  }
