# OpenMasu Event & Metric Contract v0.4

Contract release: `0.4.0`.

This document is normative for the v0.4 schemas, registries, fixtures, and reference evaluators. It defines synthetic, vendor-neutral measurement behavior; it does not define a production ingestion service.

## Serialization and versioning

- JSON Schemas use Draft 2020-12.
- Schema identifiers use `urn:openmasu:schema:<artifact>:v0.4`.
...
`registries/processing-purposes-v0.4.json` closes the public purpose vocabulary:
...
Click-to-install time (CTIT) is `install_begin_at_server - redirector_click_at`, using the OpenMasu redirector clock and the Play server install-begin clock. It is distinct from the threshold-free Play ordering check between `referrer_click_at_server` and `install_begin_at_server`. The fixture server context carries a closed `click_injection_policy`; its default threshold classifies a nonnegative CTIT strictly below 10 seconds as `click_injection_suspected`. Exactly 10 seconds is outside that category. A negative CTIT is a clock diagnostic and is never classified as injection. This public rule demonstrates deterministic contract behavior only. Production thresholds, features, and response policy remain deployment-private, versioned controls.

A fraud decision never rejects ingestion. Received evidence is stored and counted regardless of any decision; only derived attribution and `fraud_policy: net` metric runs are affected. A `source`-scoped decision describes a source-day and MUST NOT be interpreted as a decision about any individual record within it. Absence of `fraud_policy` means `gross`.
...
5. validates all 689 golden output artifacts;
6. runs named assertions for all 53 scenarios and 26 acceptance criteria (AC01-AC26);
...
- Fixture 53 exercises the additive event envelope with synthetic direct-link evidence. Schema `$id` values, artifact `contract_version` values, and all existing fixture goldens remain unchanged.

### v0.4.0 identity migration

- The project and public contract namespace move to OpenMasu and `openmasu`; every schema `$id` uses `urn:openmasu:schema:<artifact>:v0.4`.
- Contract-owned `contract_version`, event `schema_version`, `reason_code_version`, and `difference_reason_version` values advance to `0.4.0`. Independent metric-definition, rule-bundle, policy, producer, and wrapper versions retain their v0.3 values.
- `fixtures/v0.3/` moves to `fixtures/v0.4/`, registry filenames move from `*-v0.3.json` to `*-v0.4.json`, and generated public type names move from the former `V03` prefix to `OpenMasu*V04`.
- Attribution, metric, reconciliation, ordering, hashing, privacy, and fraud behavior are unchanged. The immutable pre-rename baseline is the `contract-v0.3.6` Git tag, and the mechanical comparison is documented in `docs/contract-v0.4-migration.md`.
...
- The closed custom-event vocabulary admits the exact reserved `openmasu.conversion_value_updated` key. The iOS SDK emits it only when conversion-value logging is explicitly enabled; it does not alter attribution or metric semantics.
...
- The in-place v0.3 contract used `:v0.3` schema identifiers, `0.3.0` object versions, v0.3 registries, and `fixtures/v0.3/`; the immutable v0.2.1 baseline is the `contract-v0.2.1` Git tag.
