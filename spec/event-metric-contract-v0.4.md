...
The public contract exposes only the decision, action, high-level reason category, evidence type/digest/access class, rule-bundle digest, and evaluation time. Synthetic fixtures exercise `bot_prefetch`, `replay_suspected`, `click_injection_suspected`, `click_flooding_suspected`, `referrer_time_inconsistent`, `device_integrity_failed`, and the diagnostic `ctit_clock_anomaly` through this envelope. Replay suspicion is a fraud-decision category, not a substitute for `duplicate_delivery` or idempotency classification.

Click-to-install time (CTIT) is `install_begin_at_server - redirector_click_at`, using the OpenMasu redirector clock and the Play server install-begin clock. It is distinct from the threshold-free Play ordering check between `referrer_click_at_server` and `install_begin_at_server`. The active registered fraud-bundle revision supplies the threshold; a nonnegative CTIT strictly below it is `click_injection_suspected`. A negative CTIT emits `ctit_clock_anomaly` with `decision: clear` and `action: allow`, never injection. For each tenant/app/UTC day, the negative count is divided by the install count across all source cells. When that ratio is greater than the registered `ctit_negative_rate_threshold`, every otherwise valid Install Referrer attribution whose matched redirector click falls on that day is superseded by a `provisional` result. A zero-install day does not trigger the guard.
