...
export function renderDashboard(view: DashboardView): string {
  ...
  const reportNavigation = selected
    ? `<nav aria-label="Report views"><a href="/dashboard/apps/${encodeURIComponent(selected)}">Cohorts and activity</a> <a href="/dashboard/apps/${encodeURIComponent(selected)}/differences">Stored difference audit</a></nav>`
    : "";
