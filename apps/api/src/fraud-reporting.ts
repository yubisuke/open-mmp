...
export function parseFraudAuditQuery(params: URLSearchParams): {
  ...
  if (!/^[a-z0-9][a-z0-9_-]{2,63}$/.test(appId)) throw new FraudAuditQueryError("app_id_invalid");
  ...
  return withTenant(pool, identity.tenantId, async (client) => {
    const result = await client.query<FraudAuditRow>(
      `SELECT aggregate.metric_date::text,aggregate.campaign_id,aggregate.network,aggregate.site_id,
       ...
       FROM ledger.source_day_aggregates aggregate
       LEFT JOIN ledger.fraud_decisions decision
       ...
       WHERE aggregate.tenant_id=$1 AND aggregate.app_id=$2
         AND aggregate.metric_date >= $3::date AND aggregate.metric_date < $4::date
